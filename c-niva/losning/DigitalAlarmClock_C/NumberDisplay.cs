using System;

namespace DigitalAlarmClock_C
{
    /// <summary>
    /// Representerar en nummerdisplay.
    /// </summary>
    public class NumberDisplay
    {
        #region Fält

        /// <summary>
        /// Det maximala värdet för numret.
        /// </summary>
        private int _maxNumber;

        /// <summary>
        /// Det aktuella numret.
        /// </summary>
        private int _number;

        #endregion

        #region Konstruktorer

        /// <summary>
        /// Initierar en ny instans av NumberDisplay med ett maximalt nummer.
        /// </summary>
        /// <param name="maxNumber">Ett maximalt värde ett Numberdisplay-objekt kan anta.</param>
        public NumberDisplay(int maxNumber)
            : this(maxNumber, 0)
        {
        }

        /// <summary>
        /// Initierar en ny instans av NumberDisplay med ett maximalt nummer och ett nummer.
        /// </summary>
        /// <param name="maxNumber">Ett maximalt värde ett Numberdisplay-objekt kan anta.</param>
        /// <param name="number">Ett aktuellt värde för Numberdisplay-objektet.</param>
        public NumberDisplay(int maxNumber, int number)
        {
            this.MaxNumber = maxNumber;
            this.Number = number;
        }

        #endregion

        #region Egenskaper

        /// <summary>
        /// Hämtar eller tilldelar aktuellt NumberDisplay-objekts maxnummer.
        /// </summary>
        public int MaxNumber
        {
            get { return this._maxNumber; }

            set
            {
                if (value <= 0)
                {
                    throw new ArgumentException("Värdet måste vara större än 0.");
                }
                this._maxNumber = value;
            }
        }

        /// <summary>
        /// Hämtar eller tilldelar aktuellt NumberDisplay-objekts nummer.
        /// </summary>
        public int Number
        {
            get { return this._number; }

            set
            {
                if (value < 0 || value > this._maxNumber)
                {
                    throw new ArgumentException(String.Format("Värdet är inte i intervallet 0-{0}.", this._maxNumber));
                }
                this._number = value;
            }
        }

        #endregion

        #region Instansmetoder

        /// <summary>
        /// Ökar värdet det aktuellt NumberDisplay-objektets har med värdet 1.
        /// </summary>
        public void Increment()
        {
            this.Number = (this.Number + 1) % (this._maxNumber + 1);
        }

        /// <summary>
        /// Konverterar värdet av det aktuella NumberDisplay-objektet till motsvarande 
        /// strängrepresentation med hjälp av det angivna formatet.
        /// </summary>
        /// <param name="format">En standardformatsträng för nummer.</param>
        /// <returns>En sträng representerande värdet av det aktuella NumberDisplay objektet.</returns>
        public string ToString(string format)
        {
            if (String.IsNullOrEmpty(format))
            {
                format = "G";
            }

            switch (format.ToUpperInvariant())
            {
                case "G":
                case "0":
                    return String.Format("{0}", this.Number);

                case "00":
                    return String.Format("{0:00}", this.Number);

                default:
                    throw new FormatException(String.Format("Formatsträngen {0} stöds inte.", format));
            }
        }

        #endregion

        #region Överskuggade instansmetoder

        /// <summary>
        /// Returnerar ett värde som anger om denna instans är lika med ett angivet objekt.
        /// </summary>
        /// <param name="obj">Objektet att jämföra denna instans med.</param>
        /// <returns>true om värdet är en instans av NumberDisplay motsvarar värdet av denna instans; annars false.</returns>
        public override bool Equals(object obj)
        {
            if (!(obj is NumberDisplay))
            {
                return false;
            }

            NumberDisplay other = (NumberDisplay)obj;

            return this._number == other._number && this._maxNumber == other._maxNumber;
        }

        /// <summary>
        /// Returnerar hashkoden för denna instans.
        /// </summary>
        /// <returns>Ett 32-bitars heltal hashkod.</returns>
        public override int GetHashCode()
        {
            return (this.ToString("G") + this._maxNumber.ToString()).GetHashCode();
        }

        /// <summary>
        /// Konverterar värdet av det aktuella NumberDisplay-objektet till motsvarande strängbeteckning.
        /// </summary>
        /// <returns>En sträng representerande värdet av det aktuella NumberDisplay-objektet.</returns>
        public override string ToString()
        {
            return this.ToString("G");
        }

        #endregion

        #region Överlagring av operatorer

        /// <summary>
        /// Avgör om två instanser av NumberDisplay är lika.
        /// </summary>
        /// <param name="a">Första objektet att jämföra.</param>
        /// <param name="b">Andra objektet att jämföra.</param>
        /// <returns>true om a och b representerar samma nummer; annars false.</returns>
        public static bool operator ==(NumberDisplay a, NumberDisplay b)
        {
            return a.Equals(b);
        }

        /// <summary>
        /// Avgör om två instanser av NumberDisplay inte är lika.
        /// </summary>
        /// <param name="a">Första objektet att jämföra.</param>
        /// <param name="b">Andra objektet att jämföra.</param>
        /// <returns>true om a och b inte representerar samma nummer; annars false.</returns>
        public static bool operator !=(NumberDisplay a, NumberDisplay b)
        {
            return !a.Equals(b);
        }

        #endregion
    }
}
